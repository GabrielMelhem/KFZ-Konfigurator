...
const Zusammenfassung = () => {
  ...
  return (
    <div className="mx-auto max-w-4xl w-full p-5 border rounded shadow-lg mt-10">
      <h2 className="text-3xl font-bold mb-4 text-center md:text-left">Zusammenfassung Ihrer Konfiguration</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {bestellung.fahrzeug && (
        <div className="mb-3">
          <h3 className="text-xl font-semibold">Fahrzeug:</h3>
          <p>{bestellung.fahrzeug.marke} {bestellung.fahrzeug.modell}</p>
      ...
      {bestellung.motorleistung && (
        <div className="mb-3">
          ...
          <p>{bestellung.motorleistung.motor_name} - {bestellung.motorleistung.leistung} PS</p>
      ...
      {bestellung.sonderausstattung && bestellung.sonderausstattung.length > 0 && (
        <div className="mb-3">
          <h3 className="text-xl font-semibold">Sonderausstattungen:</h3>
          <ul>
            {bestellung.sonderausstattung.map((ausstattung, index) => (
              <li key={index}>{ausstattung.sonderausstattung_name} - Preis: {ausstattung.preis} €</li>
            ))}
          </ul>
        </div>
      )}
